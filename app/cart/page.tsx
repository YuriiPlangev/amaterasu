  ...
  const handleSubmitOrder = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        alert(`Помилка: ${data.error || 'Не вдалося оформити замовлення'}`);
      }
    } catch (error: any) {
        ...
        <div className="max-w-[1920px] w-full mx-auto px-10 sm:px-6 lg:px-[144px]">
          <div className="max-w-2xl mx-auto bg-white border border-[#D8D8D8] rounded-lg p-8 text-center">
            <div className="mb-6">
              ...
              <h1 className="text-3xl font-bold text-black mb-2">Замовлення успішно оформлено!</h1>
              {orderNumber && (
                <p className="text-lg text-gray-600">
                  Номер замовлення: <span className="font-semibold text-[#9C0000]">#{orderNumber}</span>
                </p>
              )}
            ...
            <div className="bg-white border border-[#D8D8D8] rounded-lg p-6 sticky top-4">
              ...
              {!showCheckout ? (
                ...
                <form onSubmit={handleSubmitOrder} className="space-y-4">
                  ...
                  <textarea
                    name="notes"
                    placeholder="Примітки до замовлення (необов'язково)"
