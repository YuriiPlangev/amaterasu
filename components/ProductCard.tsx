...
export default function ProductCard({ product, variant = 'default' }: { product: any; variant?: 'default' | 'catalog' }) {
  ...
  return (
    <article
      className={`relative border border-[#D8D8D8] px-2 py-2 sm:px-5 sm:py-4 flex flex-col rounded-lg sm:rounded-2xl items-center gap-4 transition-all duration-300 hover:shadow-[0px_8px_16px_0px_#00000014] h-full ${
            ...
            >
              ...
              {!isInCart && !isCustomDesign && (
                <span className="relative w-6 h-6 shrink-0">
                  <Image src="/svg/shopping-bag.svg" alt="" fill className="absolute inset-0 opacity-100 transition-opacity duration-300 ease-in-out group-hover:opacity-0" />
                  <Image src="/svg/shopping-bag-red.svg" alt="" fill className="absolute inset-0 opacity-0 transition-opacity duration-300 ease-in-out group-hover:opacity-100" />
                </span>
              )}
            </button>
            {!isCustomDesign && (
            ...
            <div className="flex flex-col items-center w-full gap-[10px]">
              <div className="flex justify-between items-start gap-2 w-full">
                <div className="flex flex-col justify-start">
                  <p className={`text-[#9C0000] font-semibold text-sm line-through opacity-60 leading-none mb-1 ${hasDiscount ? '' : 'invisible'}`} aria-hidden={!hasDiscount}>{regularPrice.toFixed(2)} ₴</p>
                ...
                <p className={`hidden sm:block font-semibold text-[14px] text-right self-center ${isInStock ? 'text-[#2E7900]' : 'text-[#9C0000]'}`}>{isInStock ? tCard('inStock') : tCard('outOfStock')}</p>
              </div>
